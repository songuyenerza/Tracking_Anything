...
def detect_frame_error(box_obj_list, ratio_object):
    frame_list = []
    area_box_list = []
    H_box_list = []
    W_box_list = []
    ...
    for box_dict in box_obj_list:
        ...
        ratio_box_list.append(round(h_box/w_box, 3))
    ...
    for i in range(len(area_box_list)):
        if area_box_list[i] > 0.85 * area_avr and 0.9 * ratio_object < ratio_box_list[i] < 1.2 * ratio_object:

            frame_list_update.append(frame_list[i])
...
def preprocess_dict_frame(dict_label_frame, direction):
    ...
    for ids in dict_label_frame.keys():
        ...
        if direction == 1:
            check_w_box_end = max(W_box_list)
        else:
            check_w_box_end = w_vid * 0.85 -  min(W_box_list)
        if check_w_box_end < w_vid * 0.4 or  len(frame_list) < 7 or max(W_box_list) - min(W_box_list) <  w_vid * 0.177: 
...
def cutout_track(dict_label_frame , direction, w_vid):
    dict_label_frame_new = {}
    count = 0
    for ids in dict_label_frame.keys():
        box_obj_list = dict_label_frame[ids]
        ...
        for i, box_dict in enumerate(box_obj_list):
            ...
            if i > 1:
                ...
                box_check = box_obj_list[i-1]['box'][0] 
                if dist > w_vid * 0.2 and box[0] * direction  < direction * (box_obj_list[i-1]['box'][0]) * 0.8 and    0.5 * direction * (direction-1) + direction * box_check > 0.625 * w_vid:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_video", default="/media/anlabadmin/setup/dungtd/carton/video_test/2004008_02_IMG_0837.MOV")
    parser.add_argument("-o", "--output_save", default="/media/anlabadmin/Data/SonG/moving-recog/results_carton/")
    parser.add_argument("-s", "--size_video_out", type = int , default = [640, 360] )
    ...
    size_vid_output =( args.size_video_out[0], args.size_video_out[1])

    path_save_track = os.path.join(data_folder, str(name_video))
    ...
    vid_writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*'MJPG'), 30, size_vid_output)
    ...
    for id in data.keys(): #id = id box
        ...
        if num_data >= 10:
            ...
            if dist > 220 and direction * direction_check >= 0:
                size_data = [0 , 0]
                for ob in objs:
                    frame_idx_str = ob["fr"]
                ...
                if objs[0]["fr"] > end_frame + 40 and direction * box_check[2] > direction * w_vid /2 and direction * ob_start[2] < direction * box_check[2] *0.9:
                # if box_check[2] > 1000:
                    # count_object -= 1
    ...
    dict_label_frame_ = preprocess_dict_frame(dict_label_frame, direction)
    ...
    dict_label_frame2 = cutout_track(dict_label_frame1, direction, w_vid)
    ...
    folder_results_ = os.path.join(path_save_track,'results_carton') 
    if not os.path.exists(folder_results_):
        os.makedirs(folder_results_)

    folder_results = os.path.join(folder_results_, name_video)
    ...
    while (1):
        ...
        for key_text in list_putext.keys():
            if list_putext[key_text]["count"] < 120:
                ...
                y_size += 70
        frame_rsize =  cv2.resize(frame, size_vid_output)
